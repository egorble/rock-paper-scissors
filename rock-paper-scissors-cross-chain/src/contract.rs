...
impl Contract for RockPaperScissorsContract {
            ...
            Operation::JoinRoom { room_id } => {
                ...
                match leaderboard_chain {
                    Some(leader_chain) => {
                        ...
                        eprintln!("[JOIN_ROOM] Sent JoinRoom request to leaderboard chain {:?}", leader_chain);
                    }
                    None => {
    ...
    async fn execute_message(&mut self, message: Self::Message) {
        ...
        match message {
            GameMessage::JoinRoom { room_id, player_chain } => {
                eprintln!("[MESSAGE] Processing JoinRoom: {} from {:?}", room_id, player_chain);
                ...
                if let Ok(Some(mut room)) = self.state.rooms.get(&room_id).await {
                    if room.can_join(player_chain) {
                        if room.add_player(player_chain) {
                            ...
                            eprintln!("[MESSAGE] Player {:?} joined room {} successfully", player_chain, room_id);
                        }
                    } else {
    ...
    async fn rebuild_global_leaderboard(&mut self) {
            ...
            Ok(player_chains) => {
                ...
                for player_chain in player_chains {
                    if let Ok(Some(stats)) = self.state.player_stats.get(&player_chain).await {
                        ...
                        eprintln!("[LEADERBOARD] Added {:?} with {} wins to rebuild list", player_chain, stats.games_won);
                    }
                }
            }
            Err(_) => {
        ...
        for (i, entry) in top_100.iter().take(10).enumerate() {
            eprintln!("[LEADERBOARD] #{}: {:?} - {} wins, {:.2}% win rate ({} total games)", 
                i + 1, entry.chain_id, entry.wins, entry.win_rate(), entry.total_games);
        }
